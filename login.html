...
function required(e) {
  ...
  if (!e.target.value && !v.length) {
    e.target.parentNode.insertAdjacentHTML('afterend', '<h2 class"req" style="color: red">required</h2>');
  } else {
...
function userMatch(e) {
  ...
  if (pass.value.length >= 1) {
    if (!v.test(pass.value) && !all.length) {
      ...
      e.target.parentNode.insertAdjacentHTML('afterend', '<h5 class"req" style="color: red">Minimum of eight characters, at least one uppercase letter, one lowercase letter and one number</h5>');
...
function match(e) {
  ...
  if (!all.length) {
    if (confirm.value !== pass.value && !v.length) {
      e.target.parentNode.insertAdjacentHTML('afterend', '<h3 class="pass" style="color: red">password must match</h3>');
      return false;
    } else {
      v.forEach(x => x.remove())
      return true;
...
function matchUser(e) {
  ...
  if (!all.length) {
    ...
    if (confirm.value.length > 8 && confirm.value == pass.value) {
      if (!v.length) {
        e.target.parentNode.insertAdjacentHTML('afterend', '<h3 class="pass" style="color: red">username already exists</h3>');
        return false;
      } else {
        v.forEach(x => x.remove())
        return true;
...
function successful(e) {
  let all = Array.from(document.querySelectorAll('input')).every(x => x.value)
  let users = {
    username: user.value,
    password: pass.value
  }
  ...
  if (!usernames.includes(user.value)) {
    if (confirm.value === pass.value) {
      if (all) {
        ...
        document.querySelector('h1').textContent = "User's registration was successful🥳"
      ...
      if (!all.length) {
        if (confirm.value !== pass.value && !v.length) {
          confirm.parentNode.insertAdjacentHTML('afterend', '<h3 class="pass" style="color: red">password must match</h3>');
          return false;
        }
        else {
          v.forEach(x => x.remove())
    ...
    if (usernames.includes(user.value) && !v.length) {
      user.parentNode.insertAdjacentHTML('afterend', '<h3 class="pass" style="color: red">username already exists</h3>');
    } else {
      v.forEach(x => x.remove())
    }
  }
